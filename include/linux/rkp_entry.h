...
#define   TIMA_DEBUG_LOG_START  0x9CE00000
...
#define rkp_is_pg_protected(va)	rkp_is_protected(__pa(va), (u64 *)rkp_pgt_bitmap)
#define rkp_is_pg_dbl_mapped(pa) rkp_is_protected(pa, (u64 *)rkp_map_bitmap)
static inline u8 rkp_is_protected(u64 pa, u64 *base_addr)
{
	...
	if((phys_addr < PHYS_OFFSET) || (phys_addr  > RKP_PHYS_OFFSET_MAX)) {
		return 0;	
	}
